...
public class JsonReportEntry {
	...
	public JsonReportEntry(long timestamp, UUID runId, Long taskId,
			String taskname, String lang, Long invocId, String key,
			String rawValue) {
	...
	public JsonReportEntry(UUID runId, Long taskId, String taskname,
			String lang, Long invocId, String key, String rawValue) {
		this(System.currentTimeMillis(), runId, taskId, taskname, lang,
				invocId, key, rawValue);
	}
	...
	public JsonReportEntry(UUID runId, Long taskId, String taskname,
			String lang, Long invocId, String key, JSONObject obj) {
		this(System.currentTimeMillis(), runId, taskId, taskname, lang,
				invocId, key, obj);
	}
...
//	public JsonReportEntry(Invocation invoc, String key, String value)
//			throws NotDerivableException {
//		this(UUID.fromString(invoc.getDagId()), (long) invoc.getTaskNodeId(),
//				invoc.getTaskName(), invoc.getLangLabel(),
//				invoc.getSignature(), key, value);
//	}
	...
	public JSONObject getValueJsonObj() throws JSONException {
		...
		if (!isValueJson())
			throw new RuntimeException(
					"Value is not a JSON object, but a string.");
	...
	public String getValueRawString() {
		...
		if (!isValueString())
			throw new RuntimeException(
					"Value is not a string, but a JSON object.");
	...
	public void setKey(String key) {
		...
		if (key.contains("\n"))
			throw new RuntimeException(
					"Key must not contain newline character.");

		if (key.contains("\""))
			throw new RuntimeException(
					"Key must not contain double quote character.");

		if (key.contains("\\"))
			throw new RuntimeException(
					"Key must not contain backslash character.");
	...
	public void setValueFromJsonObj(JSONObject obj) {
		...
		if (obj == null)
			throw new NullPointerException(
					"Value JSON object must not be null.");
	...
	public void setTaskname(String taskname) {
		...
		if (taskname.contains("\n"))
			throw new RuntimeException(
					"Taskname id must not contain newline character.");

		if (taskname.contains("\""))
			throw new RuntimeException(
					"Taskname id must not contain double quote character.");

		if (taskname.contains("\\"))
			throw new RuntimeException(
					"Taskname must not contain backslash character.");
	...
	public String toString() {
		...
		if (hasTaskname())
			buf.append(ATT_TASKNAME).append(':').append("\"").append(taskname)
					.append("\",");

		if (hasLang())
			buf.append(ATT_LANG).append(':').append("\"").append(lang)
					.append("\",");
